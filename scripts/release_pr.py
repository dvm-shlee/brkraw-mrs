...
def update_pyproject_version(version: str) -> None:
    ...
    pyproject_updated, count = re.subn(
        r"^version\s*=\s*['\"][^'\"]+['\"]\s*$",
        f'version = "{version}"',
        pyproject_text,
        flags=re.MULTILINE,
...
def update_pyproject_classifiers(status_label: str) -> None:
    pyproject_text = PYPROJECT_PATH.read_text(encoding="utf-8")
    ...
    if count == 0:
        return
    if count != 1:
        raise SystemExit("Failed to update Development Status classifier in pyproject.toml")
    PYPROJECT_PATH.write_text(pyproject_updated, encoding="utf-8")


def determine_status(version: str) -> str:
    v = version.lower()
...
def update_version_files(version: str) -> None:
    ...
    update_pyproject_classifiers(determine_status(version))
...
def main() -> int:
    ...
    if not args.allow_dirty and not args.dry_run:
        ...
        logger.warning("Warning: dirty working tree allowed (dry-run or --allow-dirty).")
    ...
    release_prep_group = [INIT_PATH, README_PATH, PYPROJECT_PATH]
    commit_if_changed(
